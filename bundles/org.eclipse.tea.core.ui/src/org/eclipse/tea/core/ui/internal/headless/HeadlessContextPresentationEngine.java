 ...
 *     SSI Schaefer - suppress warnings, add "real" event loop
 ******************************************************************************/
...
public class HeadlessContextPresentationEngine implements IPresentationEngine {
	...
	public Object createGui(MUIElement element, Object parentWidget, IEclipseContext parentContext) {
		...
		if (element instanceof MContext) {
			...
			if (createdContext == null) {
				String contextName = element.getClass().getInterfaces()[0].getName() + " eclipse context"; //$NON-NLS-1$
				createdContext = (parentContext != null) ? parentContext.createChild(contextName)
	...
	public Object run(MApplicationElement uiRoot, IEclipseContext appContext) {
		// force progress service init.
		PlatformUI.getWorkbench().getProgressService();

		// even though we're not really really rendering, we need to spin the
		// event loop...
